  ...
  <div>
    <div id="header">
      ...
      <div id="account-info" v-if="logged_in">
        <a class="menu-bar" href="/upload">
          <div id="upload">Загрузить</div>
        </a>
        <a class="menu-bar" href="#">
          <div id="nickname" @click="nickname_click">{{login}}</div>
        </a>
        <a class="menu-bar" href="#">
          <div id="signout" @click="signout">Выйти</div>
        </a>
      </div>
      <div id="login-bar" v-if="!logged_in">
        <a class="menu-bar" href="#">
          <div id="signup" onclick="document.getElementById('signup_form').style.display='block'">Регистрация</div>
        </a>
        <a class="menu-bar" href="#">
          <div id="signin" onclick="document.getElementById('signin_form').style.display='block'">Вход</div>
        </a>
      </div>
    </div>
    ...
    <div id="signup_form" class="modal">
      ...
      <form class="modal-content">
        <div class="container">
          <h1>Регистрация</h1>
          <div class="control-group" id="reg_result_success" style="display: none; color: green">
            Успешно
          </div>
          ...
          <div class="clearfix">
            ...
            <button type="button" class="signupbtn" @click="register">Создать аккаунт!</button>
          </div>
        </div>
      </form>
    </div>
              ...
              xhr.onreadystatechange = function () {
                ...
                if (xhr.readyState == 4) {
                  ...
                  if (xhr.responseText == "Created") {
                    ...
                    document.getElementById("reg_result_success").style.display = "none";
                    document.getElementById("reg_result_fail").style.display = "block";
                  }
                }
              }
            } else {
